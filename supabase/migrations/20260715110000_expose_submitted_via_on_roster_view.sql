-- Expose submission source on the assignment roster view so UI can mark
-- instructor-created non-submission stubs in score cells.
...
CREATE VIEW public.submissions_with_grades_for_assignment_nice WITH (security_invoker = 'true') AS
 ...
 SELECT swe.user_role_id AS id, swe.class_id, swe.assignment_id,
    ...
    s.repository, s.sha, s.submitted_via,
